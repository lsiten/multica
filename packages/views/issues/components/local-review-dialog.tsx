...
export function LocalReviewDialog({ request, onClose }: { request: LocalReviewRequest; onClose: () => void }) {
  ...
  const pending = repositories.isFetching || branches.isFetching || manifest.isFetching || operation.isPending || indexBusy || selectionBusy;
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !operation.isPending && !indexBusy && !selectionBusy) onClose(); }}>
    <DialogContent className="flex h-[85vh] w-[95vw] max-w-6xl flex-col sm:max-w-6xl">
      ...
      {data && <>
        ...
        {confirmMerge && <div role="alert" className="flex flex-wrap items-center justify-between gap-2 rounded-sm border p-3 text-caption">
