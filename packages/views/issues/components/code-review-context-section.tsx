...
export function CodeReviewContextSection({ issueId }: { issueId: string }) {
  ...
  const candidates = tasks.filter((task) => task.work_dir || task.durable_work_dir).toSorted((a, b) => b.created_at.localeCompare(a.created_at));
  const task = candidates.find((task) => task.id === selected) ?? candidates[0];
  const inventory = useQuery({ queryKey: ["local-review-task-worktrees", workspaceId, issueId], queryFn: localReviewInventory, retry: false });
  const repositories = inventory.data?.find((row) => row.taskId === task?.id && row.workspaceId === workspaceId)?.repositories ?? [];
  const path = repositories.find((path) => path === repository) ?? repositories[0] ?? task?.work_dir;
  return <section className="space-y-2 rounded-lg border bg-card p-3">
    ...
    {!task ? <p className="text-caption text-muted-foreground">{t(($) => $.local_review.no_worktrees)}</p> : <>
      ...
      <Button className="w-full" variant="outline" size="sm" disabled={!path} onClick={() => setOpen(true)}>{t(($) => $.local_review.open)}</Button>
      {open && path && <LocalReviewDialog key={task.id + path} request={{ task_id: task.id, workspace_id: workspaceId, runtime_id: task.runtime_id ?? undefined, path, target: "main" }} onClose={() => setOpen(false)} />}
    </>}
  </section>;
}
