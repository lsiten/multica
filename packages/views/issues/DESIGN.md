...
The task run selector contains only successfully verified repository-bearing runs, never raw historical work_dir candidates. Collapse identical repository sets within one runtime to the newest verified run; keep distinct runtimes separate. If none qualify, show checking, unavailable, or empty feedback without a run selector or MR launch control.
