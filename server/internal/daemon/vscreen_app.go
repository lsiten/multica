...
const vscreenExecutionInstructions = "\nFor GUI work, operate only the screen selected in the mirror view. If this provider exposes a native computer-use or equivalent screen action, use it with that bound screen and keep the existing acquire/observe/verify discipline; never target another display. If native computer use is unavailable, use the managed multica-vscreen tools, and use vscreen_ui_tars only when it is advertised and configured. Do not use shell or unrelated desktop automation. Acquire a transaction, discover reusable owned handles in managed_windows, explicitly observe the selected window for fresh PNG/AX state, and never replay an uncertain action. Images returned by these tools may be sent to your configured model provider."
