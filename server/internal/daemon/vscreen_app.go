...
const vscreenExecutionInstructions = "\nFor GUI work, operate only the screen selected in the mirror view. You MUST perform the requested action through an actual screen tool before replying. If this provider exposes a native computer-use or equivalent screen action, call it with that bound screen and keep the existing acquire/observe/verify discipline; never target another display. If native computer use is unavailable, acquire a transaction and call the managed multica-vscreen tools, using vscreen_ui_tars for visual actions. Do not merely describe steps, claim an attempt, or use shell/unrelated desktop automation. For multi-step requests, continue observe/action/observe until the requested state is visible or a concrete tool error is returned. Discover reusable owned handles in managed_windows, explicitly observe the selected window for fresh PNG/AX state, and never replay an uncertain action. Images returned by these tools may be sent to your configured model provider."
