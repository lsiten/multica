...
func buildMirrorChatPrompt(task Task, chatPrompt string) string {
	...
	b.WriteString("Interpret references to \"the current screen\", \"this screen\", or \"当前屏幕\" as this exact bound screen. Treat the user's message below as the action to perform on it. You MUST perform the action through an actual screen tool before replying: if your runtime exposes native computer use or an equivalent screen tool, call it on this bound screen; otherwise start by acquiring a transaction and observing this bound screen, then call the managed multica-vscreen tools, using vscreen_ui_tars for the visual action. Do not merely describe the steps or claim that an action was attempted.\n")
	b.WriteString("For multi-step requests such as opening an app, keep executing observe/action/observe until the requested state is visible or a tool returns a concrete failure. A successful tool call alone is not completion; verify the resulting screen and report what was actually observed.\n")
