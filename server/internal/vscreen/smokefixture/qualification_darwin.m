...
int multica_input_qualification_run(const char *raw) {
 @autoreleasepool {
  ...
  NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(100,100,640,480)styleMask:NSWindowStyleMaskTitled|NSWindowStyleMaskClosable|NSWindowStyleMaskResizable backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;window.animationBehavior=NSWindowAnimationBehaviorNone;window.title=[cfg[@"InteractiveScratch"]boolValue]?@"Multica manual typing qualification":@"Multica input qualification (test-owned)";
  ...
  void (^publish)(void)=^{
   ...
   id focused = [NSApp accessibilityFocusedUIElement]; BOOL actual = focused == editor && editor.isAccessibilityFocused;
