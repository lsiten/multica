...
func buildMirrorChatPrompt(task Task, chatPrompt string) string {
	...
	b.WriteString("Interpret references to \"the current screen\", \"this screen\", or \"当前屏幕\" as this exact bound screen. Treat the user's message below as the action to perform on it. If your runtime exposes native computer use or an equivalent screen tool, use it on this bound screen; otherwise start by acquiring a transaction and observing this bound screen for fresh visual/accessibility state, then use the managed multica-vscreen tools or its UI-TARS visual fallback.\n")
