...
import type { LocalReviewRequest, LocalReviewSnapshot } from "@multica/core/types/local-review";

vi.mock("@multica/core/auth", () => ({ useAuthStore: (select: (state: { user: { id: string } }) => unknown) => select({ user: { id: "viewer" } }) }));
vi.mock("../../platform/local-review", () => ({ readLocalReview: async (request: LocalReviewRequest): Promise<LocalReviewSnapshot> => ({
  id: "entry-snapshot", path: request.path, branch: "feature", target: request.target,
  head: "head", target_head: "base", base: "base", dirty: false, repositories: [], branches: ["main"], commits: "head change",
  files: [{ path: "entry.ts", status: "tracked", patch: `@@ -1 +1 @@\n-before\n+${request.task_id}-change` }],
  review: { snapshot_id: "entry-snapshot", state: "open", comment: "", merged_commit: "" },
}), localReviewInventoryPage: async () => [
