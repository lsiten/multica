...
func TestInboxListBodyPreview(t *testing.T) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			case tc.want != nil && *got != *tc.want:
				t.Fatalf("body = %d characters %q…, want %d characters",
					utf8.RuneCountInString(*got), truncateInboxPreviewForTest(*got),
	...
	t.Run("multi-byte text is cut on a character boundary", func(t *testing.T) {
		...
		if !utf8.ValidString(*got) {
			t.Fatalf("preview is not valid UTF-8: %q", truncateInboxPreviewForTest(*got))
		}
		...
		if !strings.HasSuffix(*got, "…") {
			t.Fatalf("preview does not end with an ellipsis: %q", truncateInboxPreviewForTest(*got))
		}
	})
}
...
func truncateInboxPreviewForTest(s string) string {
