...
聊天页与浮动聊天窗复用镜像的按住录音、松开转文字、上滑或 Esc 取消交互。转写结果追加到当前草稿，用户确认后才发送；切换会话或智能体会取消未完成的转写。

普通聊天不创建镜像会话或视频流。客户端向 `POST /api/agents/{id}/voice/transcribe` 提交录音，服务端校验智能体调用权限和运行时访问权限，再通过所选运行时的 daemon WebSocket 转发。音频和转写结果不写入数据库、附件或日志；用户确认发送后的文字按普通聊天消息处理。

部署要求：更新服务端、Web/桌面客户端，以及目标运行时的 daemon；无需数据库迁移。在**目标运行时机器的 daemon 启动环境**中设置 `MULTICA_VOICE_TRANSCRIBER` 为转写程序的绝对路径，然后重启 daemon。只在服务端 Docker Compose 中配置该变量不会配置远端运行时。

转写程序复用镜像约定：从 stdin 读取音频二进制，通过 `MULTICA_VOICE_MIME` 获取格式，仅在 stdout 输出 UTF-8 文本。程序自身也应避免保存或记录音频与转写内容。应用不会自动安装转写引擎或选择第三方云服务。

录音最长 60 秒、最大 512 KiB；服务端转发和运行时处理最多 25 秒，每条 daemon 连接最多并发 2 个转写请求，服务端最多 32 个。取消、断线或超时会结束请求并清理临时关联；缺少转写程序、运行时离线或版本不支持时，在输入框显示错误，不自动发送空消息。
